...
const elements = {
  openManager: null,
  totalBookmarks: null,
  totalFolders: null,  message: null,
  searchInput: null,
  clearSearch: null,
  searchResults: null
};
...
function displaySearchResults(results) {
  ...
  if (results.length === 0) {
    ...
    results.forEach(bookmark => {
      ...
      item.innerHTML = `
        <div class="search-result-title" title="${escapeHtml(bookmark.title)}">${escapeHtml(bookmark.title || bookmark.url)}</div>
        <div class="search-result-url" title="${escapeHtml(bookmark.url)}">${escapeHtml(bookmark.url)}</div>
      `;
